...
import { EmbedBuilder, Message, TextChannel, DMChannel, NewsChannel, ThreadChannel } from 'discord.js';
...
export async function handleBotInfoCommand(message: Message): Promise<void> {
    if (!message.channel || !message.channel.isTextBased()) {
        console.warn(`Command invoked in non-text-based or missing channel (ID: ${message.channelId})`);
        try {
            await message.author.send("The `!bot` command can only be used in text-based channels.");
        } catch (dmError) {
    ...
    try {
        processingMessage = await message.reply('⚙️ Fetching system information...');
    ...
    .setFooter({
        ...
        if (processingMessage) {
           await processingMessage.edit('Failed to fetch system information. Please check the console.').catch(console.error);
        } else {
            await message.reply('Failed to fetch system information. Please check the console.').catch(console.error);
        }
    }
}