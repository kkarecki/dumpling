...
client.on(Events.MessageCreate, async (message) => {
  ...
  if (message.guild && message.content.startsWith("!")) {
    ...
    if (command === "!poll") {
      console.log(
        `Detected potential !poll command from ${message.author.tag}`
      );
      await handlePollCommand(client, message).catch((error) => {
        console.error("Unhandled error in handlePollCommand:", error);
        message
          .reply(
            "An unexpected error occurred whilst processing the poll command."
          )
          .catch(console.error);
      });
      return;
      ...
      console.log(
        `[MessageCreate] Detected !bot command from ${message.author.tag}`
      );
      await handleBotInfoCommand(message).catch((error) => {
        console.error("Unhandled error in handleBotInfoCommand:", error);
        message
          .reply("An unexpected error occurred whilst fetching bot info.")
          .catch(console.error);
      });
      return;
      ...
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .setDescription(
            "Here’s a list of available commands, separated by access level:"
          )
          .addFields(
            {
              name: "👤 User Commands",
            ...
            },
            {
              name: "🖥️ !client",
              value:
                "Sends the download link and minimum requirements for ariaClient.",
              inline: false,
            },
            {
              name: "\u200B",
          ...
          .setTimestamp()
          .setFooter({
            text: `Command Help Information`,
            iconURL: originalThumbnail,
          });
        ...
        await message
          .reply("An error occurred whilst generating help information.")
          .catch(console.error);
      }
      return;
      ...
      try {
        ...
        await message
          .reply("An error occurred whilst generating client information.")
          .catch(console.error);
      }
      return;
...
client.on(
  ...
  async (
    ...
    await handlePollVote(reaction as MessageReaction, user as User).catch(
      (error) => console.error("Error during handlePollVote:", error)
    );

    await handleReactionRoleAdd(
      reaction as MessageReaction,
      user as User
    ).catch((error) =>
      console.error("Error during handleReactionRoleAdd:", error)
    );
  }
);
...
client.on(Events.GuildMemberAdd, async (member: GuildMember) => {
  try {
    ...
    if (!role) {
      console.error(
        `Role with ID ${config.roleIds.new} not found in guild ${member.guild.name}`
      );
      return;
    }
...
function updateStatus() {
  ...
  if (
    ...
    !Array.isArray(config.statusMessages) ||
    config.statusMessages.length === 0
  ) {
    return;
  }
  ...
  if (typeof status !== "string") {
    console.error(
      "Invalid status message (not a string) at index",
      currentIndex
    );
