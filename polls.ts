...
export async function handlePollCommand(
  ...
  if (rawArgs.length === 0) {
    await message.reply(
      'Missing duration, question and options. Usage: `!poll [duration e.g., 1h30m] <question or "quoted question"> <option1> [option2 ...]`'
    );
    return;
  }
  ...
  if (parsed !== null) {
    ...
    console.log(
      `No duration specified or first argument is not a valid duration.`
    );
  }

  if (rawArgs.length < 3) {
    await message.reply(
      "Missing question and/or options. You need a question and at least two options."
    );
    return;
  }
  ...
  const firstQuotedIndex = rawArgs.findIndex(
  ...
  if (firstQuotedIndex === 0) {
    ...
    if (
      ...
      console.warn(
        `Question "${question}" was not quoted and contains spaces. Interpretation might be ambiguous.`
      );
    }
  }
  ...
  if (options.length > pollEmojis.length) {
    await message.reply(
      `The maximum number of options for a poll is ${pollEmojis.length}.`
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `Created poll (ID: ${pollMessage.id}) in channel ${message.channel.name} by ${message.author.tag}`
    );
    ...
    if (message.deletable) {
      await message
        .delete()
        .catch((err) =>
          console.error("Failed to delete the !poll message:", err)
        );
    }
  } catch (error) {
    ...
    await message
      .reply(
        "An error occurred whilst creating the poll. Please check the bot's permissions."
      )
      .catch(console.error);
  }
}
